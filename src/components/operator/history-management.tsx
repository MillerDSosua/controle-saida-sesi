    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
      <div className="flex flex-col lg:flex-row gap-5 items-center justify-between bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
        ...
        <div className="hidden lg:flex items-center gap-3 px-4 py-2 rounded-xl bg-slate-50 border border-slate-100">
          <Badge variant="outline" className="bg-white border-slate-200 text-slate-400 font-black uppercase text-[9px] tracking-[0.2em] px-3 py-1 rounded-md shadow-xs">
      ...
      <Card className="overflow-hidden border border-slate-100 shadow-sm bg-white rounded-[2rem] animate-in fade-in duration-700">
            ...
            <TableBody>
              {filteredHistory.length > 0 ? (
                filteredHistory.map((h) => (
                  <TableRow key={h.id} className="hover:bg-slate-50/50 transition-all duration-300 group">
                    <TableCell className="py-4 pl-8">
                      <div className="flex items-center gap-4">
                        <div className={cn("h-10 w-10 rounded-full flex items-center justify-center border transition-all duration-300", h.tipo === 'escolar' ? "bg-orange-50 text-orange-500 border-orange-100" : "bg-primary/5 text-primary border-primary/10")}>
                ...
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-20 text-slate-300 font-bold italic tracking-tight">Nenhuma atividade registrada no período.</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
        ...
        <div className="md:hidden divide-y divide-slate-100">
          {filteredHistory.length > 0 ? (
            filteredHistory.map((h) => (
              <div key={h.id} className="p-6 space-y-4 hover:bg-slate-50/50 transition-all duration-200">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className={cn("h-12 w-12 rounded-full flex items-center justify-center border transition-all shadow-sm", h.tipo === 'escolar' ? "bg-orange-50 text-orange-500 border-orange-100" : "bg-primary/5 text-primary border-primary/10")}>
                      {h.tipo === 'escolar' ? <Bus size={20} /> : <User size={20} />}
                    </div>
                    <div>
                      <h4 className="font-black text-slate-900 tracking-tight text-lg">{h.nomeExibicao || h.escolarNome}</h4>
                      <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
                        {h.tipo === 'escolar' ? "Transporte" : `Turma ${h.turmaNome}`}
                      </span>
                    </div>
                  </div>
                  <Badge className={cn("font-black uppercase text-[9px] tracking-[0.15em] px-2 py-0.5 border-none shadow-sm rounded-md", h.status === "Chamado" ? "bg-green-500 text-white" : "bg-slate-100 text-slate-400")}>
                    {h.status}
                  </Badge>
                </div>
                <div className="flex items-center justify-between pt-3 border-t border-slate-50">
                  <div className="flex items-center gap-2 text-primary bg-primary/5 px-2 py-1 rounded-lg">
                    ...
                    <span className="text-[10px] font-black tabular-nums tracking-tight">{h.dataHoraChamado ? format(h.dataHoraChamado.toDate(), "HH:mm") : "-"}</span>
                  </div>
            ...
            <div className="py-20 text-center text-slate-300 font-bold italic tracking-tight">Vazio.</div>
          )}
        </div>
      </Card>
    </div>
